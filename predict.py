...
class Predictor:
    def __init__(self, model='detect_hand', path='models/', model_factory=detect_hand_model):
    ...
    def scale(self, landmarks: np.ndarray, shape: tuple, copy=True, current_shape: tuple = None):
        if copy:
            landmarks = landmarks.copy()

        current_shape = current_shape or tuple(self.model.input.shape[1:])

        landmarks[..., 0] *= shape[0] / current_shape[0]
        landmarks[..., 1] *= shape[1] / current_shape[1]
